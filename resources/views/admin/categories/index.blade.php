...
@php
    $pageTitle = 'الفئات';
    $pageSubtitle = 'تصنيفات المنتجات فى المتجر';
@endphp
    ...
    <div class="bg-white rounded-2xl border border-zinc-200">
        <div class="p-4 sm:p-5 border-b border-zinc-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <form method="GET" class="flex flex-1 flex-col sm:flex-row gap-3">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="ابحث باسم الفئة..."
        ...
        @if ($categories->isEmpty())
            @include('admin.partials.empty-state', ['message' => 'لا توجد فئات مطابقة لبحثك'])
        @else
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="text-zinc-500 text-right border-b border-zinc-100">
                            ...
                            <th class="px-5 py-3 font-medium">الفئة الأب</th>
                    ...
                    <tbody class="divide-y divide-zinc-100">
                        @foreach ($categories as $category)
                            <tr class="hover:bg-zinc-50/70">
                                ...
                                <td class="px-5 py-3 text-zinc-600">{{ $category->parent?->name ?? '—' }}</td>
                                <td class="px-5 py-3 text-zinc-600">{{ $category->products_count }}</td>
                                <td class="px-5 py-3">
                                    <span class="text-xs font-medium rounded-full px-2.5 py-1 {{ $category->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-zinc-100 text-zinc-500' }}">
                                        {{ $category->is_active ? 'نشط' : 'غير نشط' }}
                                    </span>
                                </td>
                                <td class="px-5 py-3">
                                    <div class="flex items-center gap-3">
                                        <a href="{{ route('admin.categories.edit', $category) }}" class="text-brand-600 hover:underline">تعديل</a>
                                        <form method="POST" action="{{ route('admin.categories.destroy', $category) }}" onsubmit="return confirmDelete(this)">
                                            @csrf
                                            @method('DELETE')
            ...
            <div class="p-4 border-t border-zinc-100">{{ $categories->onEachSide(1)->links() }}</div>
        @endif
    </div>
@endsection
